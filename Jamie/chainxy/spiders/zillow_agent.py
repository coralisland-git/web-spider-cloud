...
class ZillowSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for row in self.cities:
            ...
            url = self.request_url.replace('#text#', location).replace('#index#', '1')
            request = scrapy.Request(url=url,callback=self.parse,  meta={'url' : url , 'proxy' : random.choice(self.proxy_list), 'location' : location })
            yield request
    ...
    def parse(self, response):
        ...
        if 'captcha' in response.url.lower():
            ...
            yield scrapy.Request(url=response.meta['url'], callback=self.parse, meta={'url' : response.meta['url'], 'proxy' : random.choice(self.proxy_list) , 'location' : response.meta['location'] }, dont_filter=True) 
            ...
            for index in range(0, pages):
                ...
                url = self.request_url.replace('#text#', location).replace('#index#', str(index+1))
                yield scrapy.Request(url=url, callback=self.parse_json, meta={'url' : url , 'proxy' : random.choice(self.proxy_list) }, dont_filter=True)

    def parse_json(self, response):

        if 'captcha' in response.url.lower():

            yield scrapy.Request(url=response.meta['url'], callback=self.parse_json, meta={'url' : response.meta['url'], 'proxy' : random.choice(self.proxy_list)}, dont_filter=True) 


        else :

            data = self.get_json_data(response.body)['model']['viewModel']['boards']['boards']

            for d in data:
                url = self.base_url + d['profileLink']['href']
                yield scrapy.Request(url=url, callback=self.parse_body, meta={'url': url,'proxy' : random.choice(self.proxy_list) }, dont_filter=True)

    def parse_body(self, response):

        if 'captcha' in response.url.lower():

            yield scrapy.Request(url=response.meta['url'], callback=self.parse_body, meta={'url' : response.meta['url'], 'proxy' : random.choice(self.proxy_list)}, dont_filter=True) 
